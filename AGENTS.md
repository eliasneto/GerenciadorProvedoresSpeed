...
1. `product_owner_requirements`
2. `software_architect`
3. `database_architect`, se houver banco
4. agente executor
5. `qa_tests`
6. `reviewer`
7. `docs_writer`
...
| Modelagem de banco | `database_architect` |
...
| Atualizar documentação final | `docs_writer` |
...
3. Chamar `software_architect`
4. Chamar `database_architect`, se houver banco crítico
5. Chamar agente executor
6. Chamar `qa_tests`
7. Chamar `docs_writer` no fechamento
...
- se precisa de revisão do `database_architect`.

O agente não deve remover migrations antigas sem autorização explícita.

Se a migration puder afetar dados existentes, recomendar revisão do `database_architect` antes de aplicar em produção.
